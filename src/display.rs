...
impl Display {
    pub fn new_default() -> Display {
        ...
        Display::new(width, height, Some(cells))
    }
    ...
    fn new(width: u32, height: u32, cells: Option<Vec<Cell>>) -> Display {
        Display {
            width,
            height,
            cells: cells.unwrap_or(Vec::with_capacity((width * height) as usize)),
        }
    }
    ...
    pub fn toggle_cell(&mut self, i: usize) {
        self.cells[i] = if self.cells[i] == Cell::Dead {
            Cell::Alive
        } else {
            Cell::Dead
        };
